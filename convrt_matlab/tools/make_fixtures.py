...
``tests/fixtures/cases.json`` and ``tests/fixtures/arrays/*.mat``
    ...
    python convrt_matlab/tools/make_fixtures.py
"""
...
def _images() -> list[tuple[str, Path, int | None, str]]:
    ...
    Read from convrt_matlab/data -- the very files the MATLAB tests read -- not
    from the repository's outputs/ and dataset/, so a working-tree change there
...
def _rel(p: Path) -> str:
    """Path relative to convrt_matlab, as the MATLAB tests open it."""
