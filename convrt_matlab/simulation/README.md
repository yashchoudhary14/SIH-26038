...
> **In `convrt_matlab`:** these files are the MATLAB/Simulink side of the
> telemedicine capacity model, copied unchanged except that
> `validate_against_simpy.m` now reads the bundled `simpy_results.json` by
> default. The SimPy model and its optimiser stay in the Python repository
> (paths below refer to it). Running this needs Simulink + SimEvents, which the
