...
The Python reference implementation (`src/drscreen/` one level up) is where
training happens; this folder holds everything needed to *run* the screening
system.
...
```matlab
cd matlab          % startup.m puts everything on the path
...
├── web/                        packaged copy of ../web (written by deploy/sync_web.m, not committed)
...
├── docs/                       packaged README/RESULTS/DATASETS (written by deploy/sync_web.m, not committed)
...
the published results. [`RESULTS.md`](../RESULTS.md) holds those results.
...
Scripts in `tools/`, run with the repository's Python `.venv`:
...
```bash
.venv/Scripts/python.exe matlab/tools/export_to_matlab.py
.venv/Scripts/python.exe matlab/tools/make_fixtures.py
```
...
from public research datasets (APTOS-2019, IDRiD, DDR; see [`docs/DATASETS.md`](../docs/DATASETS.md)).
...
sensitivity and specificity are in [`RESULTS.md`](../RESULTS.md).
